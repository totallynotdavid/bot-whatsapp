async function execute(ctx) {
  ...
  if (quotedMessage) {
    ...
    return { text: '🤖 Para quitar el rol de admin a alguien, responde a su mensaje o menciónalo.' };
  }

  // Filter users who are admins
  const admins = participants.filter(p => p.isAdmin);
  const usersToDemote = userIds.filter(userId => admins.some(admin => admin.id._serialized === userId));

  if (usersToDemote.length === 0) {
    return { text: '🤖 Este usuario no es administrador.' };
  }

  try {
    await chat.demoteParticipants(usersToDemote);
    return { text: `🤖 Se ha eliminado ${usersToDemote.length} administrador(es).` };
  } catch (err) {
    console.error('Error demoting users:', err);
